...
// a SIGTERM, or a command-line tool to stop processing input if it receives a
...
func main() {
	...
	// We’ll create a channel to receive these notifications (we’ll also make one
	// to notify us when the program can exit).
...
// When we run this program it will block waiting for a signal. By typing
// ctrl-C (which the terminal shows as ^C) we can send a SIGINT signal, causing
// the program to print interrupt and then exit.
